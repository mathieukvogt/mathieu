document.addEventListener("DOMContentLoaded", function () {
  ...
  if (spiralCanvas) {
    ...
    function animate(time) {
      ...
      spiralCtx.font = "10px monospace";
      spiralCtx.textAlign = "center";
      spiralCtx.textBaseline = "middle";

      const density = "#Wabc:+-. ";
      const colors = ["black", "white", "grey", "pink"];

      const cols = Math.floor(width / 3); // Adjust character size if needed
      ...
      for (let x = 0; x < cols; x++) {
        for (let y = 0; y < rows; y++) {
          ...
          for (let i = 0; i < 3; i++) {
            ...
            const ang = -t + length(subN(st, { x: 0.5, y: 0.5 }));
            const rotated = rot(st, ang);
            st.x = rotated.x;
            st.y = rotated.y;
          }
          ...
          spiralCtx.fillText(
            char,
            x * 10 + 5, // Adjust to center the character
            y * 10 + 5
          );
        }
      }
